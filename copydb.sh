...
LOCAL_URL=$(meteor mongo --url)
MONGO_LOCAL_REGEX="mongodb:\/\/(.*)\/(.*)"
if [[ $LOCAL_URL =~  $MONGO_LOCAL_REGEX ]]
then
  LOCAL_MONGO_DOMAIN="${BASH_REMATCH[1]}"
  LOCAL_MONGO_DB="${BASH_REMATCH[2]}"

  mongorestore -h $LOCAL_MONGO_DOMAIN --db $LOCAL_MONGO_DB dump/$MONGO_DB

  echo "mongorestore --drop -h ${LOCAL_MONGO_DOMAIN} --db ${LOCAL_MONGO_DB} ${MONGO_DB}"
else
  echo "Restore failed"
  exit 1
fi